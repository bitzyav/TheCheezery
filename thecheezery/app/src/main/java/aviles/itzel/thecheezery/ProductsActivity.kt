package aviles.itzel.thecheezery
...
class ProductsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var img_title: ImageView = findViewById(R.id.img_title)
        ...
        var list_view: ListView = findViewById(R.id.list_view) as ListView

        if(selected.equals("colddrinks")){
            img_title.setImageResource(R.drawable.colddrinks)
            var adapter: ProductsAdapter = ProductsAdapter(this, coldDrinks)
            list_view.adapter = adapter
        }

        if(selected.equals("hotdrinks")){
            img_title.setImageResource(R.drawable.hotdrinks)
            var adapter: ProductsAdapter = ProductsAdapter(this, hotDrinks)
            list_view.adapter = adapter
        }

        if(selected.equals("sweets")){
            img_title.setImageResource(R.drawable.sweets)
            var adapter: ProductsAdapter = ProductsAdapter(this, sweets)
            list_view.adapter = adapter
        }

        if(selected.equals("salties")){
            img_title.setImageResource(R.drawable.salties)
    ...
    private class ProductsAdapter: BaseAdapter{
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
            ...
            var desc = view.findViewById(R.id.product_description) as TextView
